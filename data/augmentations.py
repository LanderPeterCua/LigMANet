...
class Crop(object):
    """This class crops a quadrant from the original image"""

    def __init__(self,
                 train=True,
                 crop_ratio=0.5):
        """Class constructor for Crop
    ...
    def __call__(self,
        ...
        if self.train:
            ...
            crop_size = (int(img.size[0]*self.crop_ratio), int(img.size[1]*self.crop_ratio))
            rand = random.random()
            ...
            if rand < 0.25:
                ...
                dx = int(img.size[0]*self.crop_ratio)
                dy = 0
            elif rand < 0.75:
                dx = 0
                dy = int(img.size[1]*self.crop_ratio)
            else:
                dx = int(img.size[0]*self.crop_ratio) 
                dy = int(img.size[1]*self.crop_ratio) 

            img = img.crop((dx, dy, crop_size[0]+dx ,crop_size[1]+dy))
            label = label[dy:(crop_size[1]+dy), dx:(crop_size[0]+dx)]
...
class Augmentations(object):
    ...
    def __init__(self,
        ...
        self.augment = Compose([Crop(train=train, crop_ratio=crop_ratio),
                                ChangeBrightness(delta=brightness),
